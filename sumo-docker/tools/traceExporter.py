...
def procFCDStream(fcdstream, options):
    ...
    for q in fcdstream:
        ...
        tz = humanReadableTime.astimezone().tzinfo
        humanReadableTime = humanReadableTime.replace(hour=hours, minute=minutes, second=seconds, microsecond=0, tzinfo=tz)
        humanReadableTime = humanReadableTime.isoformat()
        #humanReadableTime = str(now.year) + ":" + str('{:02d}'.format(now.month)) + ":" + str('{:02d}'.format(now.day)) + " " + str(time.strftime("%H:%M:%S", time.gmtime(lt)))
        # modified by Jakob Smretschnig <jakob.smretschnig@tum.de>
        # END
        ...
        lastExported = lt
        e = FCDTimeEntry(humanReadableTime) # modified by Jakob Smretschnig <jakob.smretschnig@tum.de>
        if q.vehicle:
